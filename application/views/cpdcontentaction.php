		...
		<div class="row">
			<div class="col-md-8">
				<div class="card">
					...
					<div class="content">
						<form method="post" action="<?php echo base_url();?>admincontrol/cpdcontentaction"  enctype="multipart/form-data">
							<div class="row">
								<div class="col-md-12">
							...
							<div class="row">
								<div class="col-md-12">
									<div class="form-group">
										<label>Body</label>
									<?php if ($action == "new") { ?>
										<textarea id="full-featured" name="body" rows="5" class="form-control" placeholder="Body" value=""><?php echo set_value('body'); ?></textarea>
									<?php } if ($action == "edit") { ?>
										<textarea name="body" rows="5" class="form-control" placeholder="Body"><?php echo set_value('body',$getdata['body']); ?></textarea>
									<?php } ?>
									</div>
								</div>
							</div>
									...
									<?php } if ($action == "edit") { ?>
										<select name="sectionid" id="sectionid">
											...
											<?php foreach($sectiondata as $row1){?><option value="<?php echo $row1['id'];?>" 
												<?php echo set_select('sectionid', $row1['id']); if($row1['id']==$getdata['cpdsectionid']){ ?> selected <?php } ?>><?php echo $row1['name']; ?></option>
											<?php } ?>
										</select>
									<?php } ?>
						...
						<?php } ?>
							...
							<a href="<?php echo base_url();?>admincontrol/cpdcontentaction"><button type="button" class="btn btn-warning btn-fill pull-left">Back</button></a>
