		...
		<div class="row">
			<div class="col-md-8">
				<div class="card">
					<div class="header">
					<?php if ($action == "new") { ?>
						<h4 class="title"><?php echo $toolboxname['content']; ?> - <?php echo $toolbox1name['content']; ?> -Content</h4>
					<?php } if ($action == "edit") { ?>
						<h4 class="title"><?php echo $toolboxname['content']; ?> - <?php echo $toolbox1name['content']; ?> -Update Content</h4>
					<?php } ?>
					</div>
					<div class="content">
						<form method="post" action="<?php echo base_url();?>admincontrol/newtoolboxtalks2action"  enctype="multipart/form-data">							
							<div class="row">
								<div class="col-md-12">
									<div class="form-group">	
										<input name="toolboxid" type="hidden" class="form-control" placeholder="Product Guides" value="<?php echo $toolboxid; ?>">
										<input name="toolbox1id" type="hidden" class="form-control" placeholder="Product Guides" value="<?php echo $toolbox1id; ?>">
									</div>
								</div>
							</div>
							<div class="row">
							...
							<div class="pdfdiv hiddentrue">
								...
								<div class="row">
									<div class="col-md-12">
										...
										<?php if ($action == "new") { ?>
											<textarea id="basic-example" name="pdfdescription" rows="5" class="form-control" placeholder="Description" value=""><?php echo set_value('pdfdescription'); ?></textarea>
										<?php } if ($action == "edit") { ?>
											<textarea name="pdfdescription" rows="5" class="form-control" placeholder="Description"><?php echo set_value('pdfdescription',$getdata['description']); ?></textarea>
										<?php } ?>
										</div>
									</div>
								</div>
								<div class="row">
									<div class="col-md-12">
										...
										<?php } if ($action == "edit") { 
											if($getdata['type'] =='2' && $getdata['file'] !=''){ ?>
							...
							<div class="textimage hiddentrue">
								...
								<div class="row">
									<div class="col-md-12">
										<div class="form-group">
											<label>Description</label>
										<?php if ($action == "new") { ?>
											<textarea id="basic-example" name="description" rows="5" class="form-control" placeholder="Description" value=""><?php echo set_value('description'); ?></textarea>
										<?php } if ($action == "edit") { ?>
								...
								<div class="col-md-12">
									<div class="form-group">
										<label>Image</label>
									<?php if ($action == "new") { ?>
										<input name="imagefile" type="file" class="form-control imageupload" placeholder="Image" value="<?php echo set_value('imagefile'); ?>">
									<?php } if ($action == "edit") { 
										if($getdata['image'] != '' && $getdata['type'] =='1'){
											?>
										<img src="<?php echo base_url().'images/'.$getdata['image'].''; ?>" height="50" width="50">
									<?php }?>
						...
						<div class="btndiv hiddentrue">	
							...
							<a href="<?php echo base_url();?>admincontrol/newtoolboxtalks2list/<?php echo $toolboxid; ?>/<?php echo $toolbox1id; ?>"><button type="button" class="btn btn-warning btn-fill pull-left">Back</button></a>
...
CKEDITOR.replace( 'pdfdescription', {
	width: 450,
    height: 300,
    resize_dir: 'both',
    resize_minWidth: 200,
  ...
  filebrowserUploadUrl: "<?php echo base_url().'common/index/ckeeditorimageupload' ?>"
	...
	$(function(){
		...
		$('.imageupload').on('change', function(){
	 		if($('.imageupload').val().split('.').pop() ==='pdf' || $('.imageupload').val().split('.').pop() ==='gif' || $('.imageupload').val().split('.').pop() ==='txt'){
	 			$(".imageupload").val('');
